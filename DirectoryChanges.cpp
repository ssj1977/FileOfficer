...
*****************************************/
{
	...
	ASSERT( (DWORD_PTR)((LPBYTE)m_pCurrentRecord + ref_dwSizeOfCurrentRecord) <= m_dwBufferSize );
	...
	{//copy the m_pCurrentRecord to the beginning of m_pBuffer
		...
		ASSERT( (DWORD_PTR)m_pCurrentRecord > (DWORD_PTR)m_pBuffer + ref_dwSizeOfCurrentRecord);//will it overlap?
		__try{
...
int	CDirectoryChangeWatcher::NumWatchedDirectories()const
{
	CSingleLock lock(const_cast<CCriticalSection*>(&m_csDirWatchInfo), TRUE);
	ASSERT( lock.IsLocked() );
	int nCnt(0),max = (int)m_DirectoriesToWatch.GetSize();
	for(int i(0); i < max; ++i)
	{
...
**************************************************************/
{
	...
	m_hCompPort = CreateIoCompletionPort(hDir, 
										...
										(DWORD_PTR)pDirInfo, //the completion 'key'... this ptr is returned from GetQueuedCompletionStatus() when one of the events in the dwChangesToWatchFor filter takes place
										0);
	if( m_hCompPort == NULL )
	{
...
BOOL CDirectoryChangeWatcher::UnwatchAllDirectories()
{
	...
	if( m_hThread != NULL )
	{
		...
		int max = (int)m_DirectoriesToWatch.GetSize();
		for(int i = 0; i < max; ++i)
		{
...
//	Add the ptr to the first non-null slot in the array.
{
	...
	if( i == max )
	{
		...
		try{
			i = (int)m_DirectoriesToWatch.Add( pWatchInfo );
		}
...
CDirectoryChangeWatcher::CDirWatchInfo * CDirectoryChangeWatcher::GetDirWatchInfo(CDirectoryChangeWatcher::CDirWatchInfo * pWatchInfo, int & ref_nIdx)const
{
	...
	CSingleLock lock( const_cast<CCriticalSection*>(&m_csDirWatchInfo), TRUE);
	int i(0), max = (int)m_DirectoriesToWatch.GetSize();
	CDirWatchInfo * p;
	for(; i < max; ++i)
	{
		if( (p = m_DirectoriesToWatch[i]) != NULL
...
**********************************************/
{
	...
	PostQueuedCompletionStatus(hCompPort, sizeof(this), (DWORD_PTR)this, &m_Overlapped);//make the thread waiting on GetQueuedCompletionStatus() wake up
...
//
{
	...
	bRetVal = PostQueuedCompletionStatus(hCompPort, sizeof(CDirWatchInfo*), (DWORD_PTR)this, &m_Overlapped);
